...
func TestAdvance(t *testing.T) {
	assert := assert.New(t)
	a, err := parseAdvance(&gamefile.Play{}, "B-1")
	assert.NoError(err)
	assert.Equal("B", a.From)
	a, err = parseAdvance(&gamefile.Play{}, "1X2(64)")
	if assert.NoError(err) {
		assert.True(a.Out)
		assert.Equal([]int{6, 4}, a.Fielders)
	}
	a, err = parseAdvance(&gamefile.Play{}, "3-H(E2/TH)")
	if assert.NoError(err) {
		assert.False(a.Out)
	...
	as, err := parseAdvances(&gamefile.Play{Advances: []string{"B-1", "1-2", "2-3"}},
		PlayerID("b1"), []PlayerID{"r1", "r2"})
	assert.NoError(err)
